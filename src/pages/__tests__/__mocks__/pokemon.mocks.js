export const allMock = {
  path: '/pokemon-1/',
  location: {
    pathname: '/pokemon-1/',
    search: '',
    hash: '',
    href: 'http://localhost:8000/pokemon-1/',
    origin: 'http://localhost:8000',
    protocol: 'http:',
    host: 'localhost:8000',
    hostname: 'localhost',
    port: '8000',
    state: {
      key: '1680524006597'
    },
    key: '1680524006597',
    action: 'PUSH'
  },
  pageResources: {
    component: {},
    head: {},
    json: {
      data: {
        locales: {
          edges: [
            {
              node: {
                ns: '404',
                data: '{"Page not found":"Page not found","Sorry 😔, we couldn’t find what you were looking for.":"Sorry 😔, we couldn’t find what you were looking for.","Try creating a page in ":"Try creating a page in ","Go home":"Go home"}',
                language: 'en'
              }
            },
            {
              node: {
                ns: 'all-pokemon',
                data: '{"Pokédex":"Pokédex - English"}',
                language: 'en'
              }
            },
            {
              node: {
                ns: 'pokemon',
                data: '{"Name: ":"Name ","Genus: ":"Genus ","X: ":"Variant X ","Y: ":"Variant Y ","Number: ":"Number "}',
                language: 'en'
              }
            },
            {
              node: {
                ns: 'index',
                data: '{"What Pokémon are you looking for?":"What Pokémon are you looking for?","Welcome to Pokédex":"Welcome to Pokédex","Search":"Search","Show all":"Show all","Home Page":"Home Page"}',
                language: 'en'
              }
            }
          ]
        },
        allPokemon: {
          nodes: [
            {
              id: 'pokemon-1',
              imageUrl:
                'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/shiny/1.png',
              featuredImg: {
                childImageSharp: {
                  gatsbyImageData: {
                    layout: 'constrained',
                    placeholder: {
                      fallback:
                        'data:image/png;base64,'
                    },
                    backgroundColor: 'transparent',
                    images: {
                      fallback: {
                        src: '/static/2ec5c3271f4fdd3f9d13de05397660d9/443a8/1.png',
                        srcSet:
                          '/static/2ec5c3271f4fdd3f9d13de05397660d9/84030/1.png 50w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/3d34d/1.png 100w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/443a8/1.png 200w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/2a4a1/1.png 400w',
                        sizes: '(min-width: 200px) 200px, 100vw'
                      },
                      sources: [
                        {
                          srcSet:
                            '/static/2ec5c3271f4fdd3f9d13de05397660d9/dd425/1.webp 50w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/59d64/1.webp 100w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/d16d0/1.webp 200w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/b137e/1.webp 400w',
                          type: 'image/webp',
                          sizes: '(min-width: 200px) 200px, 100vw'
                        }
                      ]
                    },
                    width: 200,
                    height: 200
                  }
                }
              },
              locale: [
                {
                  language: 'it',
                  genus: 'Pokémon Seme',
                  name: 'Bulbasaur',
                  details: {
                    x: 'Alla nascita gli è stato piantato sulla schiena un seme\nraro. La pianta sboccia e cresce con lui.',
                    y: 'Dopo la nascita, cresce traendo nutrimento dal seme\npiantato sul suo dorso.'
                  }
                },
                {
                  language: 'es',
                  genus: 'Pokémon Semilla',
                  name: 'Bulbasaur',
                  details: {
                    x: 'Una rara semilla le fue plantada en el lomo al nacer.\nLa planta brota y crece con este Pokémon.',
                    y: 'Después de nacer, crece alimentándose de las\nsemillas de su lomo.'
                  }
                },
                {
                  language: 'en',
                  genus: 'Seed Pokémon',
                  name: 'Bulbasaur',
                  details: {
                    x: 'A strange seed was planted on its back at birth.\nThe plant sprouts and grows with this Pokémon.',
                    y: 'For some time after its birth, it grows by gaining\nnourishment from the seed on its back.'
                  }
                }
              ]
            }
          ]
        }
      },
      pageContext: {
        slug: 'pokemon-1',
        language: 'en',
        i18n: {
          language: 'en',
          languages: ['en', 'es', 'it'],
          defaultLanguage: 'en',
          generateDefaultLanguagePage: false,
          routed: false,
          originalPath: '/pokemon-1/',
          path: '/pokemon-1/'
        }
      },
      serverData: null
    },
    page: {
      componentChunkName: 'component---src-templates-pokemon-tsx',
      path: '/pokemon-1/',
      webpackCompilationHash: '9e1379b0374b77df3234',
      staticQueryHashes: [],
      slicesMap: {}
    },
    staticQueryResults: {}
  },
  uri: '/pokemon-1',
  data: {
    locales: {
      edges: [
        {
          node: {
            ns: '404',
            data: '{"Page not found":"Page not found","Sorry 😔, we couldn’t find what you were looking for.":"Sorry 😔, we couldn’t find what you were looking for.","Try creating a page in ":"Try creating a page in ","Go home":"Go home"}',
            language: 'en'
          }
        },
        {
          node: {
            ns: 'all-pokemon',
            data: '{"Pokédex":"Pokédex - English"}',
            language: 'en'
          }
        },
        {
          node: {
            ns: 'pokemon',
            data: '{"Name: ":"Name ","Genus: ":"Genus ","X: ":"Variant X ","Y: ":"Variant Y ","Number: ":"Number "}',
            language: 'en'
          }
        },
        {
          node: {
            ns: 'index',
            data: '{"What Pokémon are you looking for?":"What Pokémon are you looking for?","Welcome to Pokédex":"Welcome to Pokédex","Search":"Search","Show all":"Show all","Home Page":"Home Page"}',
            language: 'en'
          }
        }
      ]
    },
    allPokemon: {
      nodes: [
        {
          id: 'pokemon-1',
          imageUrl: 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/shiny/1.png',
          featuredImg: {
            childImageSharp: {
              gatsbyImageData: {
                layout: 'constrained',
                placeholder: {
                  fallback:
                    'data:image/png;base64,'
                },
                backgroundColor: 'transparent',
                images: {
                  fallback: {
                    src: '/static/2ec5c3271f4fdd3f9d13de05397660d9/443a8/1.png',
                    srcSet:
                      '/static/2ec5c3271f4fdd3f9d13de05397660d9/84030/1.png 50w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/3d34d/1.png 100w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/443a8/1.png 200w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/2a4a1/1.png 400w',
                    sizes: '(min-width: 200px) 200px, 100vw'
                  },
                  sources: [
                    {
                      srcSet:
                        '/static/2ec5c3271f4fdd3f9d13de05397660d9/dd425/1.webp 50w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/59d64/1.webp 100w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/d16d0/1.webp 200w,\n/static/2ec5c3271f4fdd3f9d13de05397660d9/b137e/1.webp 400w',
                      type: 'image/webp',
                      sizes: '(min-width: 200px) 200px, 100vw'
                    }
                  ]
                },
                width: 200,
                height: 200
              }
            }
          },
          locale: [
            {
              language: 'it',
              genus: 'Pokémon Seme',
              name: 'Bulbasaur',
              details: {
                x: 'Alla nascita gli è stato piantato sulla schiena un seme\nraro. La pianta sboccia e cresce con lui.',
                y: 'Dopo la nascita, cresce traendo nutrimento dal seme\npiantato sul suo dorso.'
              }
            },
            {
              language: 'es',
              genus: 'Pokémon Semilla',
              name: 'Bulbasaur',
              details: {
                x: 'Una rara semilla le fue plantada en el lomo al nacer.\nLa planta brota y crece con este Pokémon.',
                y: 'Después de nacer, crece alimentándose de las\nsemillas de su lomo.'
              }
            },
            {
              language: 'en',
              genus: 'Seed Pokémon',
              name: 'Bulbasaur',
              details: {
                x: 'A strange seed was planted on its back at birth.\nThe plant sprouts and grows with this Pokémon.',
                y: 'For some time after its birth, it grows by gaining\nnourishment from the seed on its back.'
              }
            }
          ]
        }
      ]
    }
  },
  pageContext: {
    slug: 'pokemon-1',
    language: 'en',
    i18n: {
      language: 'en',
      languages: ['en', 'es', 'it'],
      defaultLanguage: 'en',
      generateDefaultLanguagePage: false,
      routed: false,
      originalPath: '/pokemon-1/',
      path: '/pokemon-1/'
    }
  },
  serverData: null,
  params: {}
}
